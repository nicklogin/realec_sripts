...
def find_annotation_lines(ann):
    ann = ann.splitlines()
    output = []
    for line in ann:
        if re.search('^[AT#R][0-9]+',line) or line.startswith('*'):
            output.append(line)
        else:
            output[-1] += ' '+line
    return output
...
def get_corpus_output(path, err_tag, output_path, include_best_works=False):
    annline = re.compile('^[AT#][0-9]+.*?', re.DOTALL)
    ...
    for f,fold in getallfiles(path):
        ...
        insertions = []
        new_text = ''
        if 'best_works' in fold and not include_best_works:
            checker = False
        if f.endswith('.txt') and checker:
            ...
            with open(os.path.join(fold,tname)+'.ann','r',encoding='utf-8') as t:
                ann = t.read()
            annlines = find_annotation_lines(ann)
